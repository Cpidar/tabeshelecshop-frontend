...
const ItemFull = ({ item, showBorders = true, currencyCode }: ItemProps) => {
  ...
  const { handleDeleteItem, updateCartItem } = useCart()
  ...
  const changeQuantity = async (newQuantity: number) => {
    ...
    await updateCartItem(item.id, Number(newQuantity))
  }
          ...
          <div className="flex small:flex-row flex-col gap-2">
            ...
            <div className="flex gap-x-2">
              <div className="flex gap-x-3 shadow-[0_0_0_1px_rgba(0,0,0,0.1)] rounded-full w-fit p-px items-center">
                <button
                  className="w-6 h-6 flex items-center justify-center text-neutral-600 hover:bg-neutral-100 rounded-full text-md"
                  onClick={() => changeQuantity(item.quantity - 1)}
                  disabled={item.quantity <= 1}
                >
                  -
                </button>
                <span className="w-6 h-6 flex items-center justify-center text-neutral-950 text-xs">
                  {updating ? (
                    <Spinner size="12" />
                  ) : (
                    <Input
