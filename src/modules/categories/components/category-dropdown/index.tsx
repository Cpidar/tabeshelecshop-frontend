  ...
  <>
    <SubMenuInner
      ...
      label={
        <>
          {props.label}
          <ChevronLeftIcon size={16} className="mr-auto h-3" />
        </>
      }
      shift={7}
      className="relative"
...
export default function DropdownCategories({
  ...
  return (
    <Menu
      ...
      menuClassName={menuClassName}
      menuButton={
        <MenuButton className="bg-accent hover:bg-accent/80 font-bold uppercase px-4 xl:px-6 py-2 xl:py-3 rounded shrink-0 flex items-center">
    ...
    >
      {items && items.map((item, itemIdx) =>
        item.category_children?.length > 0 ? (
          ...
          <>
            <MenuItem key={`item_${itemIdx}`} href={`/ir/categories/${item.handle}`}>
              {item.name}
            </MenuItem>
          </>
        )
